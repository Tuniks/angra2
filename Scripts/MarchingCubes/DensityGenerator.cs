...
public class DensityGenerator {
    ...
    // ============== DENSITY ON 1D BUFFER ===================

    public void GenerateMapDensity(ComputeBuffer pointsBuffer, int gridSize, float gridScale, int lod, Vector2[] noiseParameters, Vector2 center, ComputeShader cs, Vector4 function) {
        if(gridSize == 0) return;
